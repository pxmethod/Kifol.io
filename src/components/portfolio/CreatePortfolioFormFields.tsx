...
export interface CreatePortfolioFormFieldsProps {
  formData: PortfolioFormState;
  errors: Record<string, string>;
  onFieldChange: (field: keyof PortfolioFormState, value: string) => void;
...
export default function CreatePortfolioFormFields({
  formData,
  errors,
  onFieldChange,
  onTogglePrivate,
  ...
  formId,
  onSubmit,
}: CreatePortfolioFormFieldsProps) {
  ...
  useEffect(() => {
    return () => {
      if (photoThumbSrc) URL.revokeObjectURL(photoThumbSrc);
    };
  }, [photoThumbSrc]);

  const portfolioTitlePlaceholder = useMemo(() => {
    const n = formData.childName.trim();
    return n ? `${n}'s Portfolio` : "Maeve's Portfolio";
  }, [formData.childName]);
      ...
      <form id={formId} onSubmit={onSubmit} className="space-y-8">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => onFieldChange('portfolioTitle', e.target.value)}
            className={`w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-discovery-primary focus:border-transparent transition-colors text-discovery-black ${
        ...
        {/* Privacy Settings */}
        <div>
          <label className="block text-3xl font-medium text-discovery-black mb-4">Privacy settings</label>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-discovery-grey">
                  Require that visitors enter a password to view this portfolio
                </p>
              </div>
              <button
                type="button"
